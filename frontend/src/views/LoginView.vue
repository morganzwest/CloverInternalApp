...
onMounted(() => {
    if (auth.lastEmail) email.value = auth.lastEmail
    // already logged in? go straight in
    if (auth.isReady && auth.isLoggedIn) window.location.href = '/app'
})
...
const submit = async () => {
    ...
    errorMsg.value = ''
    try {
        const { user, session } = await auth.signInWithPassword(email.value.trim(), password.value, rememberEmail.value)
        if (user && session) {
...
<template>
    <section class="bg-gray-50">
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            <a class="flex items-center mb-8 text-2xl font-semibold text-gray-900">
            ...
            <div class="w-full bg-white rounded-lg shadow sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-2 md:space-y-4" @submit.prevent="submit" autocomplete="on">
                        <div>
                        ...
                        <button type="submit" :disabled="loading"
                            class="w-full text-white bg-emerald-600 hover:bg-emerald-700 focus:ring-4 focus:outline-none focus:ring-emerald-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center disabled:opacity-60">
                        ...
                        <p class="text-sm font-light text-gray-500">
                            ...
                            <a href="mailto:morgan.west@cloverhr.co.uk"
